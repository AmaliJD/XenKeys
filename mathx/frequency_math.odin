...
cent_log_scale: f64 : math.LN2 / 1200
degree_log_scale: f64 : math.LN2 / 360
semitone_log_scale: f64 : math.LN2 / 12
octave_log_scale: f64 : math.LN2

add_cents :: proc(frequency, cents: f64) -> f64
{
    return frequency * math.exp_f64(cent_log_scale * cents)
}

add_degrees :: proc(frequency, degrees: f64) -> f64
{
    return frequency * math.exp_f64(degree_log_scale * degrees)
}

add_semitones :: proc(frequency, semitones: f64) -> f64
{
    return frequency * math.exp_f64(semitone_log_scale * semitones)
}

add_octaves :: proc(frequency, octaves: f64) -> f64
{
    return frequency * math.exp_f64(octave_log_scale * octaves)
}
...
{
    return frequency * math.exp_f64(get_interval_log_scale(base, divisions) * intervals)
}

@private
add_intervals_precomputed_log_scale :: proc(frequency, intervals, interval_log_scale: f64) -> f64
{
    return frequency * math.exp_f64(interval_log_scale * intervals)
}
